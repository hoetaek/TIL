...
class Solution:
    def splitArray(self, nums: list[int], m: int) -> int:
        ...
        while m > 1:
            ...
            index_max = max(range(len(nums_sum)), key=nums_sum.__getitem__)
            get_max_li = nums[index_max]

            ab_sub_li = []
            for i in range(len(get_max_li)):
                f = [sum(get_max_li[:i]), sum(get_max_li[i:])]
                ab_sub_li.append(abs(f[0]-f[1]))
            index_min = min(range(len(ab_sub_li)), key=ab_sub_li.__getitem__)

            nums[index_max:index_max+1] = [get_max_li[:index_min],
                                           get_max_li[index_min:]]
            m = m - 1

        return max([sum(i) for i in nums])
...
# print(
#     list(permutations([10, 5,  13, 4, 8,  4, 5, 11, 14, 9,  16,  10,  20, 8], 8))[1])
# print(
#     list(permutations([10, 5,  13], 2)))
